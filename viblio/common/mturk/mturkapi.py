...
from boto.mturk.question import QuestionContent,Question,QuestionForm, HTMLQuestion
...
class MTurkAPI():
    ...
    def submit_grid(self, image_urls, mt_params_config_filename):
        ...
        for i in range(2):
            ...
            for j in range(2):
                overview = Overview()
                image_content = '<img width="200" height="200" alt="image%i" align="%s" src="%s" />' % \
                (i*2+j, aligns[j], image_urls[i*2+j])
                radio_botton_1 = '<form><input type="radio" name="yes_soccer" value="yes_decision">Is that Soccer?</form>'
                
                job_content =  '%s%s' % (image_content, radio_botton_1)
                                
                each_col = each_col + '<td>%s</td>' % job_content
            all_content = all_content + '<tr>%s</tr>' % each_col
        all_content = '<html><table> %s </table></html>' % all_content
        q1 = HTMLQuestion(all_content, 5000) 
        qual = None
        
        #--------------- CREATE THE HIT -------------------
        hitid = self.mtc.create_hit(question=q1,\
