...
ALTER TABLE `dev`.`image` ADD FOREIGN KEY (video_id) REFERENCES `video` (`id`);
ALTER TABLE `dev`.`image_label` ADD FOREIGN KEY (image_id, image_video_id) REFERENCES `image` (`id`, `video_id`);
ALTER TABLE `dev`.`image_label` ADD FOREIGN KEY (detector_name) REFERENCES `detector` (`name`) ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE `dev`.`video_label` ADD FOREIGN KEY (video_id) REFERENCES `video` (`id`);
ALTER TABLE `dev`.`video_label` ADD FOREIGN KEY (detector_name) REFERENCES `detector` (`name`) ON DELETE SET NULL ON UPDATE CASCADE;
