...
ALTER TABLE `prod`.`image` ADD FOREIGN KEY (video_id) REFERENCES `video` (`id`);
ALTER TABLE `prod`.`image_label` ADD FOREIGN KEY (image_id, image_video_id) REFERENCES `image` (`id`, `video_id`);
ALTER TABLE `prod`.`image_label` ADD FOREIGN KEY (detector_name) REFERENCES `detector` (`name`) ON DELETE SET NULL ON UPDATE CASCADE;
ALTER TABLE `prod`.`video_label` ADD FOREIGN KEY (video_id) REFERENCES `video` (`id`);
ALTER TABLE `prod`.`video_label` ADD FOREIGN KEY (detector_name) REFERENCES `detector` (`name`) ON DELETE SET NULL ON UPDATE CASCADE;
