...
if test "$opencv_version" = "$check_version"
then
   echo "version is 2.4.5"
else
   echo "current version is not 2.4.5."
   echo "Do you wish to install opencv 2.4.5?"
   select yn in "Yes" "No"
   do
      case $yn in
         Yes ) 
	     echo "calling opencv install";
	     opencv_install
	     break;;
         No ) 
	     echo "terminating";
