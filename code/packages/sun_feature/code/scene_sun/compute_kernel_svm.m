...
num_classes = length(split); % number of scene classes

class_train = [];
class_test = [];
for i = 1:num_classes
    class_train = [class_train; split{i}.TrainAttr];
    class_test = [class_test; split{i}.TestAttr];
end
num_train = min(conf.max_num_train,length(split{i}.Training));
...
if exist(fullfile(conf.kernelPath, ['Test' outName]),'file')
    ...
    for i=1:num_classes
        %for j=1:min(conf.max_num_train,length(split{i}.Training))
        for j=1:length(split{i}.Training)
            if conf.imageDir
    ...
    for i=1:num_classes
        %for j=1:min(conf.max_num_test,length(split{i}.Testing))
        for j=1:length(split{i}.Testing)
            if conf.imageDir
...
if ~exist([conf.resultPath 'SVM_Result_' sprintf('%.4d',num_train) outName],'file')
    [score_train score_test] = svm_one_vs_all(K,K_test,class_train,num_train);
    for i = 1:size(score_test,2)
        label = class_test(:,i);
        if all(label) || ~any(label)
            disp('null attribute');
            continue;
        end
        score = score_test(:,i);
        [recall, precision, th, AP(i)]=precisionRecall(score, label, 'r');
        drawnow
        AP(i)
    end
    mean_ap = mean(AP)    
    disp(sprintf('#train = %.4d   Performance = %f %%',num_train, mean_ap));
    all_perf = mean_ap;
    % evaluation multi-class
%     [confidence,class_hat] = max(score_test, [], 2);
%     C = confusionMatrix(class_test,class_hat');
%     disp(sprintf('#train = %.4d   Performance = %f %%',num_train,mean(diag(C))));
%     all_perf = mean(diag(C));
    save([conf.resultPath 'SVM_Result_' sprintf('%.4d',num_train) outName],'mean_ap','score_test','score_train');
end
